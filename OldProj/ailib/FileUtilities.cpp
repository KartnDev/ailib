...
namespace ktstd
{
	...
	vector<vector<string>> FileUtilities::CSV::GetDataAsLines() const 
	{
		return this->linesOfData;
	}
	unordered_map<string, vector<string>> FileUtilities::CSV::GetDataAsLabels() const 
	{
	...
	void FileUtilities::CSV::_QueryForBytes(string filename)
	{
		...
		for (size_t i = 0; i < this->linesOfData[0].size(); i++)
		{
			...
			for (int col_index = 1; col_index < this->linesOfData.size(); col_index++)
			{
